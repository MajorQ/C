...
int choose(), plugBoard(), chooseRotor(), chooseSetting(), choosePosition(), cekInput(), resetInput();
...
int plugBoard () {
  int connection[10][2], rep, bool, i2;
  char a[2];
  system("COLOR 80");
  for (i = 0; i <= 25; i++) {
    plugOut[i] = i;
  ...
  for (rep = 0; rep <= 9; rep++) {
    ...
    printf("%79s\n\n", "Note: Only letters are accepted, letters must not be the same.");
    printf("%41s\n\n", "Press Enter to continue.");
    printf("%48s\n\n\n", "Press Escape to return to menu.");
    printf("%35s", "Connections: ");
    for (i = 0; i <= 9; i++) {
      if (connection[i][0] != -1) {
        printf("%c", connection[i][0] + 65);
      }
      if (connection[i][1] != -1) {
        printf("%c ", connection[i][1] + 65);
      }
    }
    printf("\n\n\n");
    printf("%50s", "Input: ");
    for (i = 0; i <= 1; i++) {
      do {
        ...
        a[i] = a[i] - base;
        for (i2 = 0; i2 <= 9; i2++) {
          if (a[i] == connection[i2][0] || a[i] == connection[i2][1]) {
            bool += 1;
          }
        }
      } while (bool != 0);
    ...
    for (i = 0; i <= 25; i++) {
      if (plugOut[i] == a[0]) {
        plugOut[i] = a[1];
      } else if (plugOut[i] == a[1]) {
        plugOut[i] = a[0];
      }
    }
    connection[rep][0] = a[0];
    connection[rep][1] = a[1];
    printf("\n\n%22s", " ");
    printf("%c is now connected to %c", a[0] + 65, a[1] + 65);
    Sleep(750);
    system("cls");
  }
  system("cls");
